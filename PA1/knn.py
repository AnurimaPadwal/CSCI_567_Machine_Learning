...
class KNN:
    ...
    def predict(self, features):
        ...
        for feature in features:
            ...
            count_frequency =  Counter(neighbors)
    ...
    def get_k_neighbors(self, point):
        ...
        for i in range(len(self.features)):
            ...
            distance_labels[i] = self.labels[i]
        
        distance_list = sorted(distances.items(), key = lambda x: x[1])
        

        for i in range(self.k):
            pair = distance_list[i]
            x = pair[0]
            result.append(distance_labels[x])
